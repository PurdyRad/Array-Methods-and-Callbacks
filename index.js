...
//(e) Winner of 2014 world cup final */
    const str = finals2014[0]["Win conditions"].split(' ');
    console.log(str[0]);
...
function getWinners(data, cb) {
    return cb(data).map(game => game['Away Team Goals'] > game['Home Team Goals'] ? game['Away Team Name'] : game['Home Team Name']);
...
function getAverageGoals(finalsData) {
   const finalData = finalsData.reduce(function(accumulator, item){
    return accumulator + item['Home Team Goals'] + item['Away Team Goals']
   },0);
   let n = (finalData/finalsData.length);
   return n.toFixed(2);
}
